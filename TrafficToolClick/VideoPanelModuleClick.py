...
#         frame.columnconfigure(1, weight=1)#, uniform="row")
    ...
    def _selectView(self, view_name):
        ...
        if self.view_name not in vld.view_to_lanes:
            self.debug_print("overlay lanes has not been set up")
            return
    ...
    def getResultFilePath(self):
        ...
        return os.path.join(self.output_dir, self.view_name + "_" + self.file_name + "_click") + ".xlsx"
    
    def getCanvas(self):
        return self.vid_canvas
...
#             ratio = (self.dims[0] / self.dims[1])
            ...
            if self.imgSizeFunc != None:
                self.imgSizeFunc((self.resized_tk.width(), self.resized_tk.height()))
...
#         self.frame_index = (self.frame_count - 1) - self.frame_count % self.fps
    ...
    def setImgSizeFunc(self, func):
        self.debug_print("setting image size func from input panel")
        self.imgSizeFunc = func
